...
(function() {
  ...
  SubDoc.prototype.pathEquals = function(path) {
    return pathEquals(this.path,path);
  };
  ...
  SubDoc.prototype.on = function(path, event, cb) {
    return normalizeArgs(this, arguments, function(path, event, cb) {
      return this.context.on(path, event, cb);
    });
  };
  ...
  _type.api = {
    ...
    _onOp: function(op) {
      for (var i = 0; i < op.length; i++) {
        ...
        if(c.lm !== undefined) this._updateSubdocPaths([c]);
        ...
        for (var l = 0; l < this._listeners.length; l++) {
          ...
          if (!containsPath(match_path,listener.path)) return;
          ...
          switch (listener.event) {
            ...
            case 'replace':
              if (c.li !== undefined && c.ld !== undefined) {
                cb(child_path, c.ld, c.li);
              } else if (c.oi !== undefined && c.od !== undefined) {
                cb(child_path, c.od, c.oi);
              }
              break;
            case 'move':
              if (c.lm !== undefined) {
